...
async def getChatUsers(
    ...
    try:
        ...
        if not groupInstance.isSupergroup:
            groupInstance = await getUsersByComments(
                client,
                groupInstance,
                trackUsers,
                banned_usernames,
...
async def getGroupInfo(
    client: TelegramClient, chat: Chat, supergroup: bool
) -> list[ChatFull, GroupRecord]:
    """
    ...
    if not supergroup:
        groupInstance = GroupRecord(
            ...
            group_username=getattr(
                chat, "username", None
            ),  # If private there is no username
        ...
        groupInstance = GroupRecord(
            ...
            group_title=chat.title,
            creator_id=(
                full.full_chat.creator_user_id
                if hasattr(full.full_chat, "creator_user_id")
                else None
            ),
...
async def scanUsersFromSupergroup(client: TelegramClient, groupInstance: GroupRecord):
    ...
    async for sender in client.iter_participants(groupId, limit=USER_SEARCH_LIMIT):
        ...
        userChannel = await get_channel_from_user(
            client, sender.username, groupId, sender
        )
        if userChannel:
